...
class Canvas(QtWidgets.QWidget):
    ...
    def colorByName(self, name):
        assert name in self.jingluoNames
        index = self.jingluoNames.index(name)
    ...
    def rotateShapes(self, center, angle):
        for shape in self.shapes:
            if shape.shape_type == 'point':
                shape.rotateBy(center, angle)
        self.repaint()       
    def moveSelectedShapesByoffset(self, offset):
    ...
    def drawGrid(self, p):
        ...
        # pen.setWidth(1)
        # p.setPen(QtGui.QColor(128, 128, 128))
    ...
    def buildNextLabel(self, label):
        ...
        return labelNext, color
    def findshapewithlabel(self, label, labels):
        if label in labels:
            return labels[label]
        else:
            return None
    def findNextShape(self, label, labels):
        labelNext, color = self.buildNextLabel(label)
        shape = self.findshapewithlabel(labelNext, labels)
        return shape, color
    def drawLine(self, p, shape, shapeNext, color):
        assert(len(shape.points) == 1)
        assert(shape.shape_type == 'point')
        pen = p.pen()
        pen.setColor(color)
        p.setPen(pen)
        p.drawLine(shape.points[0].x(), shape.points[0].y(), shapeNext.points[0].x(), shapeNext.points[0].y())  
    def buildLabelsSet(self):
        labels = {shape.label:shape for shape in self.shapes if shape.shape_type == 'point'}
        return labels
    def buildAllLabelsSet(self):
        labels = {shape.label:shape for shape in self.shapes}
        return labels        
    def drawlines(self, p):
        labels = self.buildLabelsSet()
        if (len(labels) == 0): 
            return
        for shape in self.shapes:
    ...
    def keyPressEvent(self, ev):
        ...
        if key == QtCore.Qt.Key_Down:
            offset = QtCore.QPointF(0, 1) 
            self.moveSelectedShapesByoffset(offset)
        ...
        if key == QtCore.Qt.Key_PageUp:
            angle = -1
            self.rotateShapes(center, angle)

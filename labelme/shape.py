...
class Shape(object):
    ...
    def rotatePoint(self, point, center, angle):
        ...
        sin = np.sin(radAngle)
        cos = np.cos(radAngle)
        rp0=[point.x() - center[0], point.y() - center[1]]
        rp = [0,0]
        rp[0] = rp0[0] * cos - rp0[1] *sin
        rp[1] = rp0[0] * sin + rp0[1] *cos
        rp1 = [rp[0] + center[0], rp[1] + center[1]]
        qrp = QtCore.QPointF(rp1[0], rp1[1]) 
        return qrp      
    def rotateBy(self, center, angle):
        self.points = [self.rotatePoint(p, center, angle) for p in self.points]

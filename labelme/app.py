...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.labelList.keypressedSignal.connect(self.canvas.keyPressEvent)
        ...
        tongleGrid = action(
            self.tr("&Grid tongle"),
            slot=self.tongleGrid,
            icon="eye",
            tip=self.tr("tongle Grid"),
            enabled=True,
        )
        help = action(
            self.tr("&Tutorial"),
        ...
        self.menus = utils.struct(
            ...
            models=self.menu(self.tr("&Models")),
    ...
    #     meridians = [model for model in g_modelCfg]
    #     # self.centralwidget = QtWidgets.QWidget(self)        
    #     self.modelsBox = QtWidgets.QComboBox()
    #     fileListLayout.addWidget(self.modelsBox)
    #     self.modelsBox.addItems(meridians)
    ...
    def loadNoneExistsShapes(self, shapes):
        print("loadNoneExistsShapes...")
        self._noSelectionSlot = True
        alllabels = [shape.label for shape in self.canvas.shapes]
        addedShapes=[]
        for shape in shapes:
            if shape.label not in alllabels:
                self.addLabel(shape)
                addedShapes.append(shape)
        self.labelList.clearSelection()
        self._noSelectionSlot = False
        allshapes = addedShapes + self.canvas.shapes
        self.canvas.loadShapes(addedShapes, replace=False)
        print("loadNoneExistsShapes...done!")
    ...
    def scaleShapes(self,shapes, scalex,scaley):
        for shape in shapes:
            for i, point in enumerate( shape.points):
                x = point.x() * scalex
                y = point.y() * scaley
                point1 = QtCore.QPointF(x, y)
                shape.points[i] = point1
    ...
    def loadFile(self, filename=None):
        ...
        preHeight = self.image.size().height()
        preWidth = self.image.size().width()
        curHeight = image.size().height()
        curWidth = image.size().width()

        if preWidth == 0 or preWidth == curWidth:
            scalex = 1
        else:
            scalex = curWidth / preWidth

        if preHeight == 0 or preHeight == curHeight:
            scaley = 1
        else:
            scaley = curHeight / preHeight

        self.image = image
        self.filename = filename
        if self._config["keep_prev"]:
            if scalex != 1 or scaley != 1:
                prev_shapes = self.scaleShapes(self.canvas.shapes, scalex,scaley)
            else:
        ...
        if self._config["keep_prev"]:
            self.loadNoneExistsShapes(prev_shapes)
            self.setDirty()
        else:
            self.setClean()
    ...
    def openFile(self, _value=False):
        ...
        if filename:
            self.loadFile(filename)
        self.actions.dnn_predict.setEnabled(True)
